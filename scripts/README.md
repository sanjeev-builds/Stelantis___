...
| `start-ai-demo.ps1` | Runs the RAG pipeline smoke test (`ai/src/rag_pipeline.py`) |
| `run-all.ps1` | `docker compose up --build` — everything at once |
| `clean-cache.ps1` | Wipes node_modules, `.next`, `__pycache__`, venvs, ChromaDB cache |
...
# start-ai-demo
cd ai && python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt && cd src && python rag_pipeline.py

# run-all
docker compose up --build

# clean-cache
rm -rf frontend/node_modules frontend/.next backend/.venv ai/.venv ai/.chroma
